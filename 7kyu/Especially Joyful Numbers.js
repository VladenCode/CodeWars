...
function numberJoy(n) {
  ...
  const resultArr = numberArr.reduce((a, b) => +a + +b);
  const reverseResult = String(resultArr).split('').reverse().join('');

  return resultArr * reverseResult === n ? true : false;
}

// Other solution:

function numberJoy(n) {
  let sum = Array.from(n.toString()).reduce((p, c) => p + Number(c), 0);
